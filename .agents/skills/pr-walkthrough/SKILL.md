...
## Styling
Use the neutral fallback styling tokens below for the walkthrough's visual design. They are brand-agnostic defaults and require no external brand source. A repo that wants branded output can provide a `pr-walkthrough-local` companion skill that supplies its own brand tokens, fonts, and styling guidance to apply on top of these defaults; if such a companion is present, follow it and report any branding caveats in the final response.
